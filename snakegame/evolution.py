...
pygame.init()
pygame.font.init() #폰트 모듈을 초기화
s = pygame.display.set_mode((SCREEN_SIZE * PIXEL_SIZE, SCREEN_SIZE * PIXEL_SIZE))#해상도 결정
pygame.display.set_caption('Snake') #타이틀바의 텍스트 설정

# generate 1st population
genomes = [Genome() for _ in range(N_POPULATION)] #POPULATION은 한 세대 당 염색체 수!
best_genomes = None #일단 BEST한 유전체는 없다고 한다.

n_gen = 0  #유전하는 횟수는 한 번 시행될때마다 1개씩 올린다.
while True:
  n_gen += 1

  for i, genome in enumerate(genomes): #순서와 값을 전달해준다.
    snake = Snake(s, genome=genome) #리스트로 넣고
    fitness, score = snake.run() #snake
